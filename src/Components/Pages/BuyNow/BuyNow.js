...
const BuyNow = () => {
  ...
  return (
      <div className='signup'>
        ...
        {(signup && validateemail && validatephonenumber && validatepincode && validateusername) === false ? <div className='disable-button'><p className='fill-inputs'>Details Missing Or haven't SignUp</p> <button disabled>Place Order</button></div>:<div><p className='fill-inputs'>Details Saved and Details will be automatically deleated when you try to change and double click on place order if you purchased bebore place your order</p> <button className='apply-button' onClick={handleClick}>Place Order</button></div> }
